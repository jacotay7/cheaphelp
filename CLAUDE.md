...
Agents working on cheaphelp itself must update documentation as part of any
change that affects user-visible behaviour, configuration, CLI interface, or
project structure. Documentation is a first-class part of the change, not a
follow-up.

What to update:

- **README.md** — reflect new features, changed commands, or altered setup steps.
- **`docs/` files** — e.g. `docs/configuration.md`, `docs/pipeline.md`,
  `docs/getting-started.md`, `docs/reference/`, `docs/dev-guide.md` — whenever
  the corresponding behaviour or configuration changes.
- **Inline code documentation** — add meaningful docstrings to new/changed
  public functions and classes; update type hints; add inline `#` comments where
  behaviour is non-obvious.
- **The module/responsibility table** in `CLAUDE.md`'s `## Layout` section —
  update it when files are added, removed, or renamed.
- **The test-layout note** in `CLAUDE.md`'s `## Layout` — update it if test
  files change structure.
- Do **not** update `CHANGELOG.md` — it is auto-generated by git-changelog from
  commit messages.

This section is read by `conventions.py` and injected into every agent's context
for cheaphelp's own issues, making these expectations effectively binding.
